...
class Step {
  ...
  public static function init () {
    ...
    if ($errors = array_filter (array_map (function ($path) {
        if (!file_exists ($path)) mkdir777 ($path);
  ...
  public static function cleanAsset () {
    $lasts = array ();
    merge_array_recursive (directory_map (PATH_ASSET), $lasts, rtrim (PATH_ASSET, DIRECTORY_SEPARATOR));
  ...
  public static function deleteHtmlAndTxt () {
    Step::newLine ('-', '清除檔案', count ($files = array_filter (array_map (function ($file) { return PATH . $file; }, directory_list (PATH)), function ($file) { return in_array (pathinfo ($file, PATHINFO_EXTENSION), array ('html', 'txt')); })));
  ...
  public static function writeIndexHtml () {
    ...
    Step::$questions = array_2d_to_1d (array_filter (array_map (function ($question) { shuffle ($question); return array_splice ($question, 0, 6); }, Step::$questions), function ($question) { return $question; }));
  ...
  public static function cleanTowns () {
    $lasts = array ();
    merge_array_recursive (directory_map (PATH_TOWNS), $lasts, rtrim (PATH_TOWNS, DIRECTORY_SEPARATOR));
  ...
  public static function cleanSitemap () {
    ...
    if ($errors = array_filter (array_map (function ($forder) {
        ...
        return !directory_delete ($forder, false) ? ' 目錄：' . $forder : '';
      }, $forders))) Step::error ($errors);
    
    Step::progress ('清除 Sitemap', '完成！');
  }
  public static function writeSitemap () {
    $files = array_filter (array_map (function ($file) { return $file; }, directory_list (PATH)), function ($file) { return in_array (pathinfo ($file, PATHINFO_EXTENSION), array ('html')); });
  ...
  public static function listS3Files () {
    ...
    } catch (Exception $e) { Step::error ($errors); }
  ...
  public static function initDirs () {
    ...
    if ($errors = array_filter (array_map (function ($forder) {
        ...
        return !((file_exists ($forder) && is_dir ($forder) && is_writable ($forder)) || mkdir777 ($forder)) ? ' 目錄：' . $forder : '';
      }, $forders))) Step::error ($errors);
  ...
  public static function cleanDirs () {
    ...
    if ($errors = array_filter (array_map (function ($forder) {
        Step::progress ('清除目錄');
        return !directory_delete ($forder) ? ' 目錄：' . $forder : '';
      }, $forders))) Step::error ($errors);
  ...
  public static function datas () {
    ...
    Step::$datas = array_values (array_filter (array_map (function ($weather) {
      ...
      }, directory_list (PATH_API_WEATHERS))));
