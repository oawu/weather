...
<div id='container' class='<?php echo !$frame_sides ? 'no_sides': '';?>'>
  ...
  <?php
    ...
    <form action='<?php echo base_url ('admin', 'crontab_logs');?>' method='get'<?php echo $has_search ? ' class="show"' : '';?>>
      <div class='l'>
        ...
        <input type='text' name='type' value='<?php echo isset ($columns['type']) ? $columns['type'] : '';?>' placeholder='請輸入名稱..' />
        <input type='text' name='message' value='<?php echo isset ($columns['message']) ? $columns['message'] : '';?>' placeholder='請輸入郵遞區號..' />
        <input type='text' name='run_time' value='<?php echo isset ($columns['run_time']) ? $columns['run_time'] : '';?>' placeholder='請輸入郵遞區號..' />
      </div>
  ...
  <?php if ($crontab_logs) {
          foreach ($crontab_logs as $crontab_log) { ?>
            <tr>
              <td width='120' data-title='ID'><?php echo $crontab_log->id;?></td>
              <td width='180' data-title='狀態'><?php echo $crontab_log->type;?></td>
              <td data-title='訊息'><?php echo $crontab_log->message;?></td>
              <td width='160' data-title='執行時間'><?php echo $crontab_log->run_time;?> 秒</td>
              <td width='160' data-title='開始時間' data-time='<?php echo $crontab_log->created_at;?>' class='created_at'><?php echo $crontab_log->created_at;?></td>
              <td width='60' data-title='編輯'>
                <a href='<?php echo base_url ('admin', 'crontab_logs', 'destroy', $crontab_log->id);?>' class='icon-bin'></a>
              </td>
            </tr>
    <?php }
        } else { ?>
          <tr><td colspan data-title='aa'>目前沒有任何資料。</td></tr>
  <?php }?>
      </tbody>
    </table>
